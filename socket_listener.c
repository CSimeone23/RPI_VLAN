...
void *udp_listener_thread(void *arg){
	...
	while(1){
		...
		if(t_data->thread_id == 4) {
			...
			while(1 == 1) {
				sleep(10);
				send_datagram( *(t_data->socket), payload1, 41, (struct sockaddr*) inet_addr("0.0.0.1"), slen);
				send_datagram( *(t_data->socket), payload2, 129, (struct sockaddr*) inet_addr("0.0.0.1"), slen);
			}
		}
